...
- 公開 URL:
  - Privacy Policy: `https://nyaion.tech/legal/privacy/`
  - Terms of Service: `https://nyaion.tech/legal/terms/`
  - Legal hub: `https://nyaion.tech/legal/`
- App Store や単独ドメインを持たないアプリには、原則として上記の Privacy Policy / Terms URL を設定する。
- 新しいアプリを公開する時は、`legal/privacy/index.html` の「アプリ別の取り扱い」と `legal/terms/index.html` の「アプリ別の個別条件」に行を追加する。
- 外部送信、分析、クラッシュレポート、クラウド同期、課金の有無は、アプリ実装と配布設定を確認してから断定する。
- 法務文書として公開する前に、必要に応じて専門家レビューを行う。

## デプロイ / 公開状況

- 本リポ（`gaooh/nyaion.tech`）は **Cloudflare Pages の Git 連携**で配信する。`main` への push で自動ビルド/デプロイされる（ビルド不要・出力ディレクトリ `/`）。
- 公開用 Cloudflare Pages プロジェクト: `nyaiontech-site`（**Access なしの一般公開**。社内 `_portal/` の `nyaiontech-portal` とは別物）
